...
% Communication
There has been a steady progression in the field of communication protocols available for web applications. From client-server HTTP requests (XMLHttpRequest) via client-server full-duplex TCP connections (WebSocket), it has recently become possible with peer-to-peer real-time communication (WebRTC). In section~\ref{sec:communication}, these technologies are presented. Also, issues with NAT traversal in peer-to-peer communication and how they are addressed in WebRTC are explained.
