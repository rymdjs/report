...
WebSocket (RFC 6455) is a protocol which was standarized in 2011. The protocol allows two way communication between a server and a client through persistent connections. It does not require opening multiple HTTP connections like XHR. The protocol is built on top of the TCP protocol, and is independent from the HTTP protocol. The implications of this is that the server does not need to open a new TCP connection for every incoming message as in XHR, and the high overhead from HTTP is eliminated which ease server workload \cite{RFC6455:Online}.
