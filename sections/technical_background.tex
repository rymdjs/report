...
\subsection{Namecoin}
A phenomenon that has been on the rise during recent years is that of cryptocurrencies such as Bitcoin \cite{CryptoCoinInsider:2014:Online}. Each participant in the Bitcoin network keeps a ledger of all transactions throughout the history of the network. In order for a transaction to be deemed valid, it needs to be included by one of the nodes in a cryptographically signed block together with other recent transactions and a salt. This whole block is run through an algorithm with the goal of generating a hash in a special format. If the generated value is incorrect a new salt is used. It is this brute-force search for salts generating correct hashes that are called \emph{mining} and constitutes the work done by \emph{miners} to keep the network running. As an incentive, each verified block also includes a reward to the miner that first finds it and submits it to the blockchain\cite{internetForBeginners:2014:Online}. In effect, all transactions ever made are publicly available and tracked so that anyone can confirm their validity. This prevents forgery and double-spending of bitcoins.
...
\subsection{NAT, STUN, TURN and ICE}
...
Peer-to-peer connectivity is handled by RTCPeerConnection.
...
local patterns in the language. Unlike DES who uses a Fiestel network,
AES is using a structure  similar to Rijndael – with a fixed block size.
Encryptton is preformed in rounds, the number of rounds depends on the lenght of the key. Decryption is done with the same key but inversed, it is also preformed in a itterativly.
