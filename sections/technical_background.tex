This chapter gives a theoretical foundation for the areas: client-side storage, distributed storage, communication, and cryptography. Standards and technologies for web applications are being rapidly developed, and the boundaries for what is possible to achieve in a web application are being continously pushed by browser vendors and standards groups. In \emph{Why Mobile Web Apps Are Slow} \cite{MobileApps:Online} it is argued how JavaScript is not yet suitable for heavy performing applications such as video editing or high-end 3D games, or anywhere where low-level memory management is required. Performance is not an issue for this project, as it is not included in the product's nature or scope.
