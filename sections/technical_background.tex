...
At the transport layer WebRTC makes use of UDP which can be motivated my the fact that timeliness is vital for real-time communication. The UDP protocol alone is not enough to construct efficient peer-to-peer applications, for this very reason WebRTC leverage a number of additional protocols - with UDP as the base (See figure ~\ref{fig:WebRTCProtocols}). 
