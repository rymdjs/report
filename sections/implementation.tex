...
DRAFT: \\
Encryption uses two algorithms the $RSAES-PKCS1-v1.5$ and the $AES-CBC$, both algorithms is included in the webcrypto APi and will therfore be provided by the browser.
For Shuttle all encryption/decryption is handled by the browser, and it relies (is bound to) on the presence of a working Webcrypto\cite{WebCrypto:Online} implementation.
The asymmetric keys are wraped in pkcs8 and pki cetificates, and the symmetric key is in raw format.
All have static size of $1024bits$ for Assymetric($RSAES-PKCS1-v1.5$) and $128bits$ for symmetric($AES-CBC$) and is not explicitly generated via the API.
