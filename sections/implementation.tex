...
Furthermore, because the WebCrypto API lacks functionality for storing keys between sessions, or even exporting private keys, IndexedDB is used as makeshift storage. In order to use IndexedDB for the keys they need to be parsed to certificates. This is done by the library \emph{bignumber-jt} through cryptoback.js. All certificates have a static key size where asymmetric () keys are of $1024bits$ and symmetric keys are of $256bits$; none are explicitly generated via the API.
