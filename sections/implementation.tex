...
In Rymd the encryption functionallty is all contained within a own module.
this structure was chosen for it's high modularity and
also since allot of functionallity is 3rd part is in a experimental stage.
inwokeing experimental 3rdpart code might render the Rymd library completly
useless within a short time.

Rymd handles this by wrapping all the code inside a module, that could easily be replaced when the final 3rd-part programs.
Each module contains several functions. Each function contains the code neccessary to provide its functinallity.

Among the yet experimental librarys, Rymd uses the latest version of the Web Cryptography library yet only implemented as a experimental feature in Chrome.
The Web crypto Api provides detailed implementaion guildlines, everything yet only experimental.

Among many Features WCA provides Encryption, Decryption,Singning,
Verifing and diffrent kinds of Keygeneration, All functionallity is provided without exposure the keys everything is handled in the source code browser.
At this stage Rymd uses the webcrypto api for encryption (3AES - 1024) ,decryption key import and export ,
Rymd can not benfit form the Asymmetric Encryption key-generation functionallity since the api cannot provide any key storage.
For further information of wedcrypto api
