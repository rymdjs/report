...
%TODO: Rewrite, explain "secure primitives"
...
Until recently, the practice of performing cryptographic operations in a web browser has been considered a bad practice by security professionals. A reason for this is that it has been impossible to verify the integrity of client side source code between executions - something that has now changed with the advent of signed browser extensions. Another issue is the internal openness of JavaScript - any cryptographic implementation would unavoidably expose all their primitives, as well as raw private and secret key data. This is being addressed by the new Web Cryptography API, an open standard for implementation of cryptographic primitives in web clients accessible through client code. Basically, all primitives (such as hashing, encryption, decryption, signing and generation of keys) and raw key material would be black boxed for the client application and executed natively in the web browser. It is however not yet finished and at this time of writing only Google Chrome out of the major browsers have implemented more than a basic PRG.

At this stage Rymd therefore only runs in the latest versions of Google Chrome or Chromium. % uses the webcrypto api for encryption (3AES - 1024) ,decryption key import and export ,
The implementation provided by these browsers supply key generation but there is no support for persisting keys between sessions or even exporting private keys. Until this is implemented, the Rymd crypto module therefore generates keys on its own; Doing this is bad practice and is only performed in order to get a prototype running. Shuttle lets all encryption and decryption be handled by the browser, relying on the presence of a working webcrypto\cite{WebCrypto:Online} implementation. %TODO: Not all encryption/decryption. WebRTC handles communication encryption

%TODO: Loads of references!
The Encryption uses two algorithms, namely the $RSAES-PKCS1-v1.5$ and the $AES-CBC$. Both algorithms are included in the webcrypto API and are therefore provided by the browser. %TODO Introduce the subject and expand, why these algorithms? Advantages/Disadvantages and alternatives
Furthermore, the asymmetric keys are wraped in pkcs8 and pki cetificates while the symmetric key exist in raw format.
All have static size where assymetric keys are of $1024bits$ and symmetric($AES-CBC$) are of $128bits$; none are explicitly generated via the API.

In order to support key storage between sessions, the key generation is moved out of the api and handeled by a library 'bignumber-jt'[24]; The key is then parsed to a certifacte of Asn structure, % before it's imported into its low-level interface.
Moving the key generation also makes it possible to sign and encrypt using the same key, something the native generation does not allow.

% Rymd instead generates its own keys and stores them localy as certificates. The certificates are then imported every time a new session starts.

W3C The organisation behind the webcrypto API, Have announced their intention to implement A Secure Keystore which would allow Rymd to use a more secure implementation.
% TODO Johannes
