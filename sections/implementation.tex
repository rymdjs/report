...
Furthermore, because the WebCrypto API lacks functionality for storing keys between sessions, or even exporting private keys, IndexedDB is used as makeshift storage. In order to use IndexedDB for the keys they need to be parsed to certificates. This is done by the library \emph{bignumber-jt} through cryptoback.js. All certificates have a static key size where asymmetric keys are of $1024bits$ and symmetric ($AES-CBC$) are of $256bits$; none are explicitly generated via the API.
...
% TODO Explain cipher block chaining below
\subsection{Algorithms}
RymdCrypto uses two algorithms for encryption namely the $RSAES-PKCS1-v1.5$ and the $AES-CBC$. AES-CBC is a fast and simple to use symmetric key based on AES using cipher block chaining\cite{AESISFAST:Online}. $RSAES-PKCS1-v1.5$ on the other hand is an AES implementation based on the RSA algorithm.

For the signing algorithm the working group behind the WebCrypto API recommends $RSASSA-PKCS1-v1_5$. This choice is further motivated by the lack of alternatives, there are no other RSA signing algorithms included in the WebCrypto API.

Also, a good hashing algorithm is needed in order to avoid collision of hashes, that is two generated hash values being exactly the same – similar to the birthday problem~\footnote{http://statistics.about.com/od/ProbHelpandTutorials/a/What-Is-The-Birthday-Problem.htm}. For hashing purposes RymdCrypto uses SHA-256 as explained in section~\ref{sec:creationofresources}.
...
Both $RSAES-PKCS1-v1.5$ and the DSA algorithms are included in the WebCrypto API (and therefore provided by the browser). The $RSAES-PKCS1-v1.5$ uses RSA which makes it is appropriate for Rymd since its keys can be used for both signing and encryption (se table \ref{table:webcrypoapi}). DSA algorithms are faster for signing and key generating but slower when it comes to signature checking. They also lack the ability to perform encryption.
...
When considering commercial aspects for our project RSA is clearly the winner. Commercial RSA certificates are much more widely deployed compared to DSA certificates. Furthermore, the asymmetric keys are wrapped in PKCS\#8 and SPKI certificates while the symmetric key exist in raw format. The Private-Key Information Syntax Standard (PKCS\#8) defines a way to store the private key, and the Simple public key infrastructure (SPKI) defines a way to store the public key. All three standards was chosen because they are the only three formats that is currently supported by Chromium \cite{ImplementedChromium:Online}.
