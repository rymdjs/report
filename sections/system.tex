...
Rymd leverages the open source project PeerJS which simplifies sending peer-to-peer data between clients. PeerJS makes use of WebRTC, it is essentially split into two components: a server which takes care of connection brokering, and a client side API which interacts with the server.
