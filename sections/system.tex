...
The core Rymd library contains no implementation meant to deal with a problem area but rather acts as the central hub with workflows that binds all parts together. The library is divided into modules that separates complex areas of the application – areas that are most likely to change independently over time – into well defined and easily exchangeable \emph{modules}. Rymd is designed to have the modules supplied dynamically through dependency injection. With key features being implemented with technologies from ever changing areas at the web's forefront, this modular design was imperative in order to make the system maintainable (as explained in section \ref{sec:modularity}).
