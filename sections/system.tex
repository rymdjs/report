...
The core Rymd library doesn't contain any implementations dealing with specific problem areas, but rather acts as the central hub with workflows binding all parts together. The library is divided into modules that separate complex areas of the application – areas that are most likely to change independently over time – into well defined and easily exchangeable \emph{modules}. Rymd is designed to have the modules supplied dynamically through dependency injection. With key features being implemented with technologies from constantly changing areas at the web's forefront, this modular design was imperative in order to make the system maintainable (as explained in section~\ref{sec:modularity}).
